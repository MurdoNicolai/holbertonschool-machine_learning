...
#     print(y_test[row])
#     print(X_test[row])
# print(len(predictions), len(y_test), len(y_test[1]), len(X_test), len(X_test[0][0]))
...
""" second machine learning algorythem using data predicting chance of getting High/Low
    increases in bitcoin value and outputting high, low or neither based on this """
...
#         model_translator.fit(X_train, y_train, epochs=5, batch_size=32, validation_split=0.2)

#         predictions = model_translator.predict(X_train)


#         firstQ = np.average(np.quantile(predictions, 0.25, axis = 0)[0])

#         lastQ = np.average(np.quantile(predictions, 0.75, axis = 0)[0])

#         new_predictions = np.ones((predictions.shape[0]))
#         for row in range(len(predictions)):
#             if predictions[row] < firstQ:
#                 new_predictions[row] = 0
#             elif predictions[row] > lastQ:
#                 new_predictions[row] = 2
#             else:
#                 new_predictions[row] = 1

#         correct = [0, 0, 0]
#         total = [0, 0, 0]
#         for row in range(len(new_predictions)):
#             if new_predictions[row] == y_train[row]:
#                 correct[int(y_train[row])] += 1
#             total[int(y_train[row])] += 1

#         accuracy = (correct[0]/total[0] + correct[2]/total[2])*2 + correct[1]/total[1]
#         if accuracy > highest_accuracy:
#             highest_accuracy = accuracy
#             highest_accuracy_model = model_translator
#             highest_correct = correct
#             highest_total = total
#         count += 1


# print(highest_correct, highest_total, highest_accuracy, count)

# highest_accuracy_model.save('my_model_translator2.keras')
# input = X_test
# answers = y_test
# isup = 0
# jsup = 0
...
#     for i in range (1,11):
#         sum_comp = min + (i * step)
#         new_predictions1 = np.ones((X_train.shape[0]))
#         new_predictions2 = np.ones((X_train.shape[0]))
#         for row in range(len(X_train)):
#             if X_train[row][0] + X_train[row][1] >= sum_comp and (X_train[row][0]/(X_train[row][0] + X_train[row][1])) >= per_comp:
#                 new_predictions1[row] = 0
#             if X_train[row][0] + X_train[row][1] < sum_comp and (X_train[row][0]/(X_train[row][0] + X_train[row][1])) >= per_comp:
#                 new_predictions2[row] = 0


#         corsup = 0
#         corinf = 0
#         for row in range(len(new_predictions1)):
#             if new_predictions1[row] == 0 and y_train[row] == 0:
#                 corsup += 3
#             elif new_predictions1[row] != 0 and y_train[row] != 0:
#                 corsup += 1
#             if new_predictions2[row] == 0 and y_train[row] == 0:
#                 corinf += 3
#             elif new_predictions2[row] != 0 and y_train[row] != 0:
#                 corinf += 1
#         if corsup > correctsup:
#             correctsup = corsup
#             isup = sum_comp
#             jsup = per_comp
#         if corinf > correctinf:
#             correctinf = corinf
#             iinf = sum_comp
#             jinf = per_comp
##
...
#             isup = sum_comp
#             jsup = per_comp
## sup: 0.562224805355072 0.48, 0.5670453429222106 0.46
#
...
#         new_predictions1 = np.ones((X_train.shape[0]))
#         for row in range(len(X_train)):
#             if X_train[row][0] + X_train[row][1] >= sum_comp and (X_train[row][1]/(X_train[row][0] + X_train[row][1])) >= per_comp:
#                 new_predictions1[row] = 2


#         corsup = 0
#         for row in range(len(new_predictions1)):
#             if new_predictions1[row] == 2 and y_train[row] == 2:
#                 corsup += 2
#             elif new_predictions1[row] != 2 and y_train[row] != 2:
#                 corsup += 1
#         if corsup > correctsup:
#             correctsup = corsup
#             isup = sum_comp
...
print(correct_best, total_best, 100*(1 + div_best / (correct_best[0] + correct_best[2])), best_rong)

# tf.keras.utils.get_custom_objects()['custom_activation'] = custom_activation

# model_translator = tf.keras.models.load_model('my_model_translator.keras')

# predictions = model_translator.predict(input)
...
#         correct[int(answers[row])] += 1
#     total[int(answers[row])] += 1
# div = correct[0]/total[0] + correct[1]/total[1] + correct[2]/total[2]
# print(correct, total, div)
